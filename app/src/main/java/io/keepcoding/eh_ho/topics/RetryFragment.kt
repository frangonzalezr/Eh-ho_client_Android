...
class RetryFragment : Fragment() {
    ...
    var retryInteractionListener : RetryFragment.RetryInteractionListener? = null
    var topicsInteractionListener: TopicsFragment.TopicsInteractionListener? = null

    override fun onAttach(context: Context) {
        super.onAttach(context)
        if (context is RetryFragment.RetryInteractionListener && context is TopicsFragment.TopicsInteractionListener) {
            retryInteractionListener = context
            topicsInteractionListener = context
        } else
            throw IllegalArgumentException("Context doesn't implement ${RetryFragment.RetryInteractionListener::class.java.canonicalName}")
    }
